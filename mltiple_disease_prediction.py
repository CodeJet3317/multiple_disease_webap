...
parkinson_model = pickle.load(open('parkinson_disease.sav','rb'))
...
if (selected == 'Diabetes Prediction'):
    ...
    with col1:
        kinThickness = st.text_input("Skin Thickness")
    ...
    if st.button('Diabetes test result'):
        diab_prediction = diabetes_model.predict([[Pregnancies,Glucose,BloodPressure,kinThickness,Insulin,BMI,DiabetesPedigreeFunction,Age]])
        if (diab_prediction[0]== 1):
            diab_diagnosis = 'The Patient is having diabetes'
        else:
            diab_diagnosis = 'The patient is not a diabetic'
...
if (selected == 'Heart Disease Prediction'):
   ...
   # creating a button for Prediction
    ...
    if st.button('Heart Disease Test Result'):
        try:
            input_data = [float(age), int(sex), int(cp), float(trestbps), float(chol), int(fbs),
            ...
            heart_diagnosis = 'Please ensure all inputs are valid numeric or mappable values.'
